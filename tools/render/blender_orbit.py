...
# ---- orbit render ----
import math as _m
cx0,cz0,cy0=8,-14,63     # development centre (world)
R=95; H=30
...
sc.cycles.samples=110; sc.cycles.use_denoising=True
sc.render.resolution_x=960; sc.render.resolution_y=540
sc.view_settings.view_transform='Filmic'; sc.view_settings.exposure=-0.2; sc.view_settings.look='Medium Contrast'
sc.render.image_settings.file_format='PNG'
N=30
for f in range(N):
    a=_m.radians(150) + (f/N)*_m.radians(150)      # sweep 150deg around the SW->E arc facing sea
    px=cx0+R*_m.cos(a); pz=cz0+R*_m.sin(a); py=cy0+H+8*_m.sin(f/N*_m.pi)
    from mathutils import Vector
    loc=Vector(w2b(px,py,pz)); tgt=Vector(w2b(cx0,cy0,cz0))
    co.location=loc; d=(tgt-loc).normalized(); co.rotation_euler=d.to_track_quat('-Z','Y').to_euler()
    sc.render.filepath="/private/tmp/claude-501/-Users-oskarpeterson-Documents-AI/fa90304f-6600-463f-8146-eb8df80e98f1/scratchpad/orbit/f%03d.png"%f
