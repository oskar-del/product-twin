...
def resolve_site(site_path, designation):
    """Resolve the site directory, designation and file paths."""
    site = Path(site_path) if site_path else DEFAULT_SITE
    ...
    desig = designation or "SVÄRTINGE 54:28"
    return site, sources, osm, out, desig


def load_frame(sources_path, osm_path):
    ...
    pd = json.loads(sources_path.read_text())["datasets"]["property_division"]
...
def ingest_gpkg(gpkg_path, origin, designation="SVÄRTINGE 54:28", buffer_m=CLIP_BUFFER_M):
    e0, n0 = origin
...
def run(zip_path=None, site_path=None, designation=None):
    site, sources, osm, out, desig = resolve_site(site_path, designation)
    origin, pd = load_frame(sources, osm)
    if zip_path is None:
...
if __name__ == "__main__":
    ...
    ap.add_argument("--zip", help="local path to fastighetsindelning zip")
    ap.add_argument("--site", help="site directory (relative to repo root or absolute)")
    ap.add_argument("--designation", help="property designation, e.g. 'DJURÖ 4:147'")
    ap.add_argument("--self-test", action="store_true")
    a = ap.parse_args()
    if a.self_test:
        self_test()
    else:
        run(a.zip, a.site, a.designation)
