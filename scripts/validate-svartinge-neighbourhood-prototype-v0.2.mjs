...
const designStudiesPath="data/sites/sweden/saterdalsvagen-14/design-candidate-studies-v0.1.json";
const designStudiesSchemaPath="config/spatial/svartinge-design-candidate-studies-v0.1.schema.json";
...
export function validateDesignCandidateStudies(studies,{scene=read(scenePath),repoRoot=root}={}){
  ...
  check(studies.schema_version==="svartinge-design-candidate-studies/v0.1"&&studies.entity_type==="DesignCandidateStudySet"&&studies.subject==="SVÄRTINGE 54:28","design candidate identity drift");
  ...
  check(policy.candidate_evidence_class==="CONCEPT"&&policy.selected_candidate_id===null&&policy.legal_effect==="NONE"&&policy.geometry_scope==="MASSING_ONLY","design candidate policy promoted");
  ...
  const volumeIds=new Set();for(const candidate of candidates){
    ...
    check(candidate.evidence_class==="CONCEPT"&&candidate.concept_storeys===1&&candidate.design_signals?.length>=3,`${candidate.candidate_id}: concept evidence or signals invalid`);
    const presentation=candidate.presentation_profile??{};exactKeys(presentation,["architectural_language","roof_form","wall_finish","roof_finish","roof_rise_m","terrace_depth_m","south_glazing_modules","presentation_only"],`${candidate.candidate_id} presentation`,check);
    check(["NORDIC_VIEW_HOUSE_CONCEPT","NORDIC_COURTYARD_HOUSE_CONCEPT","NORDIC_SPLIT_LEVEL_CONCEPT"].includes(presentation.architectural_language)&&presentation.roof_form==="SHALLOW_GABLE"&&["LIGHT_TIMBER_CONCEPT","NATURAL_TIMBER_CONCEPT","SAGE_TIMBER_CONCEPT"].includes(presentation.wall_finish)&&["CHARCOAL_METAL_CONCEPT","OXIDE_METAL_CONCEPT"].includes(presentation.roof_finish)&&presentation.presentation_only===true,`${candidate.candidate_id}: architectural presentation profile promoted or invalid`);
  ...
  const schema=read(designStudiesSchemaPath,repoRoot);check(schema.additionalProperties===false&&schema.properties?.schema_version?.const==="svartinge-design-candidate-studies/v0.1","strict design candidate schema missing");
...
export function validateSvartingePrototype(scene,{checkFiles=true,repoRoot=root}={}){
  ...
  check(terrain?.geometry?.segments>=64,"terrain mesh too coarse for continuous realistic rendering");
  ...
  const contextBuildings=elements.filter(x=>x.type==="CONTEXT_BUILDING");check(contextBuildings.length>=10&&contextBuildings.every(x=>x.evidence_class==="DERIVED"),"derived context massing incomplete or promoted");
  check(contextBuildings.every(item=>Math.abs(item.geometry.position[1]-(terrainHeight(item.geometry.position[0],item.geometry.position[2])+item.geometry.size[1]/2))<.01),"context building is not terrain-grounded");
  const roads=elements.filter(x=>x.type==="ROAD");check(roads.length>=2&&roads.every(x=>x.evidence_class==="DERIVED"),"derived road context incomplete or promoted");
  check(roads.every(item=>item.geometry.points.every(([x,y,z])=>Math.abs(y-terrainHeight(x,z))<.01)),"road ribbon is not terrain-grounded");
  ...
  const plotStage=scene.navigation?.find(x=>x.id==="PLOT_ORBIT");check(plotStage?.camera?.[1]>=70&&JSON.stringify(plotStage?.visible_groups)===JSON.stringify(["TERRAIN","PLOT","ROAD"]),"analytical plot view is cluttered or oblique");
  const previewStage=scene.navigation?.find(x=>x.id==="CONCEPT_HOUSE_ON_PLOT");check(previewStage?.visible_groups?.includes("CONCEPT_BUILDING")&&previewStage?.visible_groups?.includes("OPENING")&&!previewStage?.visible_groups?.some(type=>["ROOM","FURNITURE"].includes(type)),"concept preview visibility invalid");
