...
export function validateVisualReconstruction(manifest,{repoRoot=root}={}){
  ...
  const renderings=manifest.renderings??[];check(renderings.length===1,"visual reconstruction set incomplete");
  const item=renderings[0]??{};
  ...
  check(item.rendering_id==="VR_SE_SVARTINGE_STREET_ARRIVAL_V01"&&item.viewpoint==="STREET_ARRIVAL_EXISTING_CONDITION","visual reconstruction viewpoint drift");
  ...
  check(item.asset_path==="prototype/svartinge-neighbourhood/assets/street-arrival-existing-condition-v0.1.png"&&item.asset_sha256==="47c0ce69c29659c219f9c12d1cc1be08235a505ed941dad48e801ad0f8ccec13","visual reconstruction asset identity drift");
  check(JSON.stringify(item.pixel_dimensions)===JSON.stringify({width:1672,height:941}),"visual reconstruction dimensions drift");
  check(item.source_refs?.includes("OBS_SE_HEMNET_EXACT_ADDRESS_GALLERY")&&item.source_refs?.includes("OBS_SE_GOOGLE_STREETVIEW_NEARBY_2022_07"),"visual reconstruction source bindings incomplete");
