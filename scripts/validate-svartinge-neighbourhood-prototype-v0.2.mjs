...
export function validateDesignCandidateStudies(studies,{scene=read(scenePath),repoRoot=root}={}){
  ...
  const volumeIds=new Set();for(const candidate of candidates){
    exactKeys(candidate,["candidate_id","label","summary","evidence_class","footprint_m2","concept_storeys","approx_gross_floor_area_m2","volumes","design_signals"],candidate.candidate_id??"candidate",check);
...
export function validateSvartingePrototype(scene,{checkFiles=true,repoRoot=root}={}){
  ...
  if(checkFiles){
    ...
    if(fs.existsSync(path.join(repoRoot,viewerPath))){const html=fs.readFileSync(path.join(repoRoot,viewerPath),"utf8");check(html.includes("design-candidate-studies-v0.1.json")&&html.includes('id="designDock"')&&html.includes("buildDesignCandidates")&&html.includes("mountDesignStudies")&&html.includes("setActiveCandidate"),"versioned massing-lab candidate interface is not mounted");check(html.includes("MASSING LAB · PREVIEW ONLY")&&html.includes("0 SELECTED")&&html.includes("NO SETBACK, ACCESS, LEVEL, ENTITLEMENT OR APPROVAL CLAIM"),"massing-lab legal and selection disclosure missing");check(html.includes("candidateStudyGroup.visible=candidatePreview")&&html.includes("candidateReplacesBaseline")&&html.includes("Massing lab · 3 studies"),"candidate preview does not replace the dead generic design stage");check(html.includes("beaconFill")&&html.includes("opacity:.1")&&html.includes("blending:THREE.AdditiveBlending"),"subject-plot glow surface missing from Realistic profile");check(html.includes("new THREE.ArrowHelper")&&html.includes("RELATIVE FALL · DISPLAY ONLY"),"relative plot-fall cue is missing or presented without its limitation");check(!/data\.project_status\.selected_house_profile\s*=(?!=)/.test(html)&&!/policy\.selected_candidate_id\s*=(?!=)/.test(html),"preview interaction can silently select a design");}
