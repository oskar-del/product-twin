...
export function validateTerrainShadingViewer(html){
  ...
  check(html.includes("aerialDrapeOpacityByStage={NEIGHBOURHOOD_VIEW:.94,STREET_VIEW:.24,PLOT_ORBIT:.3,CONCEPT_HOUSE_ON_PLOT:.18}")&&html.includes("aerialDrapeMesh.visible=realistic&&!conceptComparison"),"close-view aerial drape can overpower the orbitable model or the three-scheme comparison");
  return {ok:errors.length===0,assertions,errors};
}
...
export function validateStreetApproachViewer(html){
  ...
  check(html.includes("plotBeaconLabelRole==='SUBJECT'&&['STREET_VIEW','PLOT_ORBIT'].includes(stage.id)")&&html.includes("streetBoundaryCue=true")&&html.includes("if(o.userData.plotAnalyticalCue)o.visible=streetApproach&&Boolean(o.userData.streetBoundaryCue)"),"street approach plot locator is missing boundary lights or restores obstructive analytics");
  check(html.includes("function tuneStreetBoundaryCue")&&html.includes("object.material.size=1.05")&&html.includes("base+(top-base)*.72")&&html.includes("plotBeaconMaterials.splice"),"street approach boundary lights are oversized, excessively animated, or performance-heavy");
  check(html.includes("userData.utilityInfrastructure=true")&&html.includes("if(o.userData.utilityInfrastructure)o.visible=false"),"unverified visual-only utility furniture obstructs a default camera or reads as evidence");
...
export function validateNeighbourhoodDetailViewer(html){
  ...
  check(section.includes("userData.contextBuildingDetail=true")&&html.includes("o.userData.contextBuildingDetail")&&html.includes("allowed.has('CONTEXT_BUILDING')"),"building detail can float after its parent context geometry is hidden");
  ...
  check(html.includes("roadVisualEntries=[]")&&html.includes("function mergeRoadGeometries(entries)")&&html.includes("function mountRoadNetwork()")&&html.includes("data.elements.forEach(buildElement);mountRoadNetwork();"),"source road ribbons are not deterministically draw-call consolidated");
  check(html.includes("sourceGeometry='OSM_ROAD_RIBBONS'")&&html.includes("mesh.userData.visualOnly=true;mesh.userData.evidenceEffect='NONE'")&&html.includes("Road geometry does not verify legal access, frontage, ownership or engineering condition."),"road-network visual batching can promote or invent road evidence");
  check(html.includes("tree.userData.contextVegetation=true")&&html.includes("if(o.userData.contextVegetation)o.visible=realistic&&boundedSiteStage&&!conceptComparison"),"procedural individual vegetation is rendered across the full neighbourhood or three-pane comparison");
...
export function validateContextBuildingLodViewer(html){
  ...
  check(html.includes("contextBodyLodEntries=[],contextRoofLodEntries=[]")&&section.includes("distance<170?'NEAR_0_170_M':distance<650?'MID_170_650_M':'FAR_650_PLUS_M'"),"context-building body and roof LOD bands are missing or overlap");
  check(section.match(/bands=\['NEAR_0_170_M','MID_170_650_M','FAR_650_PLUS_M'\]/g)?.length===2,"context-building body and roof mount bands are incomplete");
  check(section.includes("footprintBounds(g.points_xz)")&&section.includes("sourceFootprintGeometry(g.points_xz,g.height_m)")&&section.includes("contextBodyLodEntries.push")&&section.includes("contextRoofLodEntries.push")&&section.includes("sourceGeometry='OSM_FOOTPRINT_BOUNDS'"),"middle/far context LOD is not source-footprint bound");
  check(section.includes("function mountContextBodyLod()")&&section.includes("function mountContextRoofLod()")&&section.match(/new THREE\.InstancedMesh/g)?.length===2&&section.includes("new THREE.BoxGeometry(1,1,1)")&&section.includes("gableRoofGeometry(1,1,1)")&&section.match(/matrix\.compose\(position,quaternion,scale\)/g)?.length===2&&section.match(/mesh\.instanceMatrix\.needsUpdate=true/g)?.length===2,"middle/far context LOD is not deterministically instanced");
  check(section.match(/mesh\.userData\.visualOnly=true/g)?.length===2&&section.match(/mesh\.userData\.evidenceEffect='NONE'/g)?.length===2,"context LOD can masquerade as observed building evidence");
  check(section.includes("mesh.userData.contextDistance=distance")&&section.includes("roof.userData.contextDistance=distance")&&html.includes("boundedSiteStage=streetApproach||plotStudy||candidatePreview")&&html.includes("contextDistanceLimit=candidatePreview?150:boundedSiteStage?170:Infinity")&&html.includes("contextDistanceHidden")&&html.includes("contextLodReplaced=realistic&&item.type==='CONTEXT_BUILDING'&&!boundedSiteStage"),"street/plot/concept context geometry or realistic LOD replacement is not distance bounded");
  check(section.includes("mesh.castShadow=distance<120")&&section.includes("roof.castShadow=distance<120")&&section.match(/mesh\.castShadow=false;mesh\.receiveShadow=false/g)?.length===2&&!section.includes("new THREE.PointLight")&&!section.includes("EffectComposer"),"middle/far context LOD exceeds the bounded rendering budget");
  check(html.includes("function buildRealismDecor(){addSky();mountContextBodyLod();mountContextRoofLod();")&&html.includes("Middle- and far-distance bodies and gable caps are visual-only LOD derived from source-footprint bounds; they do not assert observed roof form, façade form or height."),"context LOD is not mounted or disclosed");
  check(html.includes("if(data)applyProfile(mode)")&&html.includes("if(mode==='COMPARE')applyProfile(name,previewCandidateId)")&&!html.includes("camera.updateProjectionMatrix();applyProfile(name,previewCandidateId);renderer.render"),"static spatial evidence profiles are recomputed on every animation frame");
  check(html.includes("renderer.shadowMap.autoUpdate=false")&&html.includes("renderer.shadowMap.needsUpdate=true")&&html.includes("candidateSolarArrows.forEach(arrow=>arrow.setDirection(solarDirection));renderer.shadowMap.needsUpdate=true")&&html.includes("if(data)applyProfile(mode);renderer.shadowMap.needsUpdate=true"),"static neighbourhood shadows are recomputed on every animation frame or fail to refresh on state changes");
  return {ok:errors.length===0,assertions,errors};
}
...
export function validateSvartingePrototype(scene,{checkFiles=true,repoRoot=root}={}){
  ...
  check(streetEye>=1.65&&streetEye<=1.85,"street camera is not at a credible local eye height");
  check(streetTargetEye>=.35&&streetTargetEye<=.75,"street approach does not reveal the road-to-plot ground relationship");
  check(streetHorizontal>=35&&streetHorizontal<=65,"street approach does not frame the plot at arrival scale");
  check(Math.hypot(streetStage?.target?.[0]-plotCenter[0],streetStage?.target?.[2]-plotCenter[1])<=15,"street approach does not centre the subject plot");
  ...
  check(namedStreetRoads.length>0&&Number.isFinite(streetRoadDistance)&&streetRoadDistance<=2.6,"street approach camera is not source-bound to the Säterdalsvägen road corridor");
