...
export function validateSvartingePrototype(scene,{checkFiles=true,repoRoot=root}={}){
  ...
  const unselectedStage=scene.navigation?.find(x=>x.id==="CONCEPT_HOUSE_ON_PLOT");check(!unselectedStage?.visible_groups?.some(type=>["CONCEPT_BUILDING","OPENING","ROOM","FURNITURE"].includes(type)),"unselected design stage leaks concept geometry");
