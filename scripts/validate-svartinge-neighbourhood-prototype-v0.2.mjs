...
export function validateDesignCandidateStudies(studies,{scene=read(scenePath),repoRoot=root}={}){
  ...
  const volumeIds=new Set();for(const candidate of candidates){
    ...
    check(["NORDIC_VIEW_HOUSE_CONCEPT","NORDIC_COURTYARD_HOUSE_CONCEPT","NORDIC_SPLIT_LEVEL_CONCEPT"].includes(presentation.architectural_language)&&["SHALLOW_GABLE","CROSS_GABLE","MONO_PITCH"].includes(presentation.roof_form)&&["LIGHT_TIMBER_CONCEPT","NATURAL_TIMBER_CONCEPT","SAGE_TIMBER_CONCEPT"].includes(presentation.wall_finish)&&["CHARCOAL_METAL_CONCEPT","OXIDE_METAL_CONCEPT"].includes(presentation.roof_finish)&&presentation.presentation_only===true,`${candidate.candidate_id}: architectural presentation profile promoted or invalid`);
...
export function validateSvartingePrototype(scene,{checkFiles=true,repoRoot=root}={}){
  ...
  const streetStage=scene.navigation?.find(x=>x.id==="STREET_VIEW"),streetEye=streetStage?.camera?.[1]-terrainHeight(streetStage?.camera?.[0],streetStage?.camera?.[2]);check(streetEye>=1.8&&streetEye<=2.6,"street camera is not at a credible local eye height");
  ...
  if(checkFiles){
    ...
    if(fs.existsSync(path.join(repoRoot,viewerPath))){const html=fs.readFileSync(path.join(repoRoot,viewerPath),"utf8");check(html.includes('id="gateDisclosure"')&&html.includes("gateDisclosure.onclick")&&html.includes("aria-expanded")&&html.includes(".gate.expanded em"),"evidence disclosure is not compact and deliberate");check(html.includes("buildNeighbourhoodPlotLocator")&&html.includes("SÄTERDALSVÄGEN 14 · SUBJECT PLOT")&&html.includes("new THREE.TorusGeometry")&&html.includes("neighbourhoodPlotLocator.visible=plotBeaconEnabled&&stage.id==='NEIGHBOURHOOD_VIEW'"),"neighbourhood-scale subject plot locator is missing");check(html.includes("NEIGHBOURHOOD_VIEW:52")&&html.includes("STREET_VIEW:48")&&html.includes("PLOT_ORBIT:48")&&html.includes("CONCEPT_HOUSE_ON_PLOT:44"),"stage-specific camera framing is missing");check(!html.includes("for(let i=0;i<145;i++")&&html.includes("item.type==='LANDCOVER'&&item.geometry.surface_class==='forest'"),"unclassified procedural vegetation obscures source-bound context");}
    ...
    if(fs.existsSync(path.join(repoRoot,viewerPath))){const html=fs.readFileSync(path.join(repoRoot,viewerPath),"utf8");check(html.includes("function candidateRoof")&&html.includes("profile.roof_form==='MONO_PITCH'")&&html.includes("roof.rotation.z")&&html.includes('id="designSignals"')&&html.includes("candidate.design_signals.slice(0,2)")&&html.includes("candidate.presentation_profile.roof_form.replaceAll"),"three concept studies do not expose distinct architectural roof and design strategies");}
