...
import { validateRoomLabMediaProof } from "./validate-room-lab-media-proof.mjs";
...
const STILL_FILES = new Map([
  ["SHOT_ROOM_QA_FRONT_V0_1", "room-qa-front.png"],
  ["SHOT_ROOM_QA_REAR_V0_1", "room-qa-rear.png"],
  ["SHOT_ROOM_QA_LEFT_V0_1", "room-qa-left.png"],
  ["SHOT_ROOM_QA_RIGHT_V0_1", "room-qa-right.png"],
  ["SHOT_ROOM_QA_TOP_V0_1", "room-qa-top.png"],
  ["SHOT_ROOM_QA_LOW_EYE_V0_1", "room-qa-low-eye.png"],
  ["SHOT_ROOM_HERO_V0_1", "room-hero.png"]
]);
...
async function runnerDigest() {
  const files = [
    "scripts/render-room-lab-media.mjs",
    "scripts/media/room-lab-renderer-browser.mjs"
  ];
...
async function preflight({ video }) {
  const validation = await validateRoomLabMediaProof();
  if (!validation.ok) throw new Error(`Media contract gate failed:\n${validation.errors.join("\n")}`);
  const [scene, orientation, renderPack] = await Promise.all([
    readJson(`${MEDIA_DIR}/scene-manifest.json`),
    readJson(`${MEDIA_DIR}/orientation-evidence.json`),
    readJson(`${MEDIA_DIR}/render-jobs.json`)
  ]);
  ...
  const job = renderPack.jobs.find((entry) => entry.job_id === "RENDER_ROOM_LAB_DIRECT3D_V0_1");
  const digest = await runnerDigest();
  if (job.status !== "READY") throw new Error(`Render job must be READY, found ${job.status}`);
  if (job.renderer.runner_state !== "IMPLEMENTED" || job.renderer.runner_version !== "ROOM_LAB_MEDIA_RUNNER_V1" || job.renderer.runner_sha256 !== digest) throw new Error("Render runner binding mismatch");
  await assertEnvironment({ video });
  return { runner_sha256: digest, job };
}
...
async function startServer() {
  ...
  const server = http.createServer(async (request, response) => {
    try {
      ...
      else if (file === "/renderer.mjs") bytes = await readFile(path.join(ROOT, "scripts/media/room-lab-renderer-browser.mjs"));
...
async function launchCapture(origin) {
  ...
  const readyExpression = `new Promise((resolve,reject)=>{const started=Date.now();const poll=()=>{if(window.roomLabMedia?.ready)return resolve(true);if(Date.now()-started>30000)return reject(new Error('renderer ready timeout'));setTimeout(poll,25)};poll()})`;
...
async function capturePng(capture, method, args, width, height) {
  ...
  const result = await evaluate(capture.cdp, `window.roomLabMedia.${method}(...${JSON.stringify(args)})`);
...
async function stillSpecs() {
  const outputs = (await readJson(`${MEDIA_DIR}/output-assets.json`)).outputs;
  const cameras = (await readJson(`${MEDIA_DIR}/camera-shots.json`)).shots;
  return [...STILL_FILES].map(([shotId, file]) => {
    const output = outputs.find((entry) => entry.shot_id === shotId && entry.job_id === "RENDER_ROOM_LAB_DIRECT3D_V0_1");
    const shot = cameras.find((entry) => entry.shot_id === shotId);
    if (!output || !shot) throw new Error(`Missing still contract for ${shotId}`);
    return { shot_id: shotId, role: shot.role, output_asset_id: output.output_asset_id, file, ...output.dimensions_px };
  });
}
...
async function captureStillsOnce(server, specs) {
  ...
  try {
    for (const spec of specs) {
      const captured = await capturePng(capture, "renderShot", [spec.shot_id, spec.width, spec.height], spec.width, spec.height);
...
async function renderStills(preflightResult, server, overwrite) {
  ...
  for (let index = 0; index < first.length; index += 1) {
    if (first[index].sha256 !== second[index].sha256 || first[index].result.pixels.rgba_sha256 !== second[index].result.pixels.rgba_sha256) throw new Error(`Determinism mismatch: ${first[index].shot_id}`);
  }
  ...
  for (const entry of first) {
    ...
    records.push({
      output_asset_id: entry.output_asset_id,
      ...
      duplicate_render_sha256: second.find((candidate) => candidate.shot_id === entry.shot_id).sha256,
      audit: entry.result.audit
    });
  }
  const run = {
    schema_version: "0.1.0",
    run_id: "ROOM_LAB_STILLS_RUN_V1",
  ...
  await writeFile(path.join(ROOT, OUTPUT_DIR, "room-lab-stills-run.json"), `${JSON.stringify(run, null, 2)}\n`);
  return run;
}
...
const renderAll = args.has("--all") || (!args.has("--stills") && !args.has("--video"));
const renderStillsRequested = renderAll || args.has("--stills");
const renderVideoRequested = renderAll || args.has("--video");
const overwrite = args.has("--overwrite");
const preflightResult = await preflight({ video: renderVideoRequested });
const server = await startServer();
try {
  const results = {};
  if (renderStillsRequested) results.stills = await renderStills(preflightResult, server, overwrite);
  if (renderVideoRequested) results.video = await renderVideo(preflightResult, server, overwrite);
  await writeFile(path.join(ROOT, MEDIA_DIR, "render-environment.json"), `${JSON.stringify({ schema_version: "0.1.0", runner_version: "ROOM_LAB_MEDIA_RUNNER_V1", runner_sha256: preflightResult.runner_sha256, environment: EXPECTED_ENVIRONMENT }, null, 2)}\n`);
  process.stdout.write(`${JSON.stringify({ ok: true, stills: results.stills?.outputs.length ?? 0, video: Boolean(results.video), provider_cost_usd: 0 })}\n`);
} catch (error) {
