...
# Derive expected stale count from the manifest: line_items + substitutions
expected_stale="$(node -e "
  import fs from 'node:fs';
  const m = JSON.parse(fs.readFileSync('data/procurement/marbella-living-room-commerce-es-29660.v1.json','utf8'));
  console.log((m.line_items?.length ?? 0) + (m.substitutions?.length ?? 0));
")"

if [[ "$stale_count" -ne "$expected_stale" ]]; then
  echo "integration gate FAIL: expected $expected_stale stale offer blockers (derived from manifest), found $stale_count" >&2
  cat "$stale_result" >&2
  exit 1
fi

git diff --check
echo "INTEGRATION SPRINT GATE PASS · current-commerce gate remains blocked by $expected_stale stale observations (derived)"
