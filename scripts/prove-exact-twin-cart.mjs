...
async function mcp(endpoint,name,args){
  const res=await fetch(endpoint,{method:'POST',headers:{'content-type':'application/json','user-agent':'product-twin-exact-cart/0.1'},body:JSON.stringify({jsonrpc:'2.0',id:crypto.randomUUID(),method:'tools/call',params:{name,arguments:{...args,meta:{'ucp-agent':{profile:PROFILE}}}}})});
...
let liveProduct=null,liveVariant=null;
for(const p of lookup?.products??[]){for(const v of p.variants??[]){if(v.id===ref.variant_id){liveProduct=p;liveVariant=v;break}}}
if(!liveVariant){
...
const wellRes=await fetch(`${origin}/.well-known/ucp`,{headers:{'user-agent':'product-twin-exact-cart/0.1'}});if(!wellRes.ok)throw new Error(`merchant /.well-known/ucp ${wellRes.status}`);const well=await wellRes.json();
...
const createdRaw=await mcp(endpoint,'create_cart',{cart:{line_items:[{quantity:1,item:{id:ref.variant_id}}]}});let cart=toolPayload(createdRaw,'cart');if(!cart?.id)throw new Error('Cart created without cart id');
...
let localized=false;
try{const updatedRaw=await mcp(endpoint,'update_cart',{id:cart.id,cart:{line_items:[{quantity:1,item:{id:ref.variant_id}}],context}});const updated=toolPayload(updatedRaw,'cart');if(updated?.id){cart=updated;localized=true}}catch{}
