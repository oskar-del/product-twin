...
const canvas = document.querySelector("#room-lab-canvas");
...
const [sceneManifest, cameraPack] = await Promise.all([
  ...
  fetch("/files/data/media/room-lab/v0.1/camera-shots.json", { cache: "no-store" }).then(assertResponse).then((response) => response.json())
]);
...
function makeCamera(shot, width, height) {
  const spec = shot.camera;
  let camera;
  if (shot.projection === "ORTHOGRAPHIC") {
...
const pathShot = cameraPack.shots.find((shot) => shot.role === "CAMERA_PATH");
const keyframes = pathShot.path.keyframes;
const positionCurve = new THREE.CatmullRomCurve3(keyframes.map((keyframe) => new THREE.Vector3().fromArray(keyframe.position_m)), false, "chordal", 0.5);
const targetCurve = new THREE.CatmullRomCurve3(keyframes.map((keyframe) => new THREE.Vector3().fromArray(keyframe.target_m)), false, "chordal", 0.5);
...
function samplePath(frame) {
  if (!Number.isInteger(frame) || frame < 0 || frame >= pathShot.path.frame_count) throw new Error(`Invalid frame ${frame}`);
  let segment = keyframes.length - 2;
  for (let index = 0; index < keyframes.length - 1; index += 1) {
    if (frame <= keyframes[index + 1].frame) {
      segment = index;
      break;
    }
  }
  ...
  const local = (frame - start.frame) / (end.frame - start.frame);
  const eased = cubicEaseInOut(local);
  const curveT = (segment + eased) / (keyframes.length - 1);
  return {
    frame,
    position_m: positionCurve.getPoint(curveT).toArray(),
    target_m: targetCurve.getPoint(curveT).toArray(),
    vertical_fov_deg: THREE.MathUtils.lerp(start.vertical_fov_deg, end.vertical_fov_deg, eased)
  };
}

for (const keyframe of keyframes) {
  const sample = samplePath(keyframe.frame);
  const values = [...sample.position_m.map((value, index) => value - keyframe.position_m[index]), ...sample.target_m.map((value, index) => value - keyframe.target_m[index]), sample.vertical_fov_deg - keyframe.vertical_fov_deg];
  if (values.some((value) => Math.abs(value) > 1e-10)) throw new Error(`Camera interpolation misses keyframe ${keyframe.frame}`);
}
...
async function renderShot(shotId, width, height) {
  const shot = cameraPack.shots.find((entry) => entry.shot_id === shotId);
  if (!shot || shot.role === "CAMERA_PATH") throw new Error(`Unknown still shot ${shotId}`);
  const camera = makeCamera(shot, width, height);
  ...
  return { shot_id: shotId, role: shot.role, width, height, camera: { position_m: camera.position.toArray(), projection: shot.projection }, camera_collision, audit, pixels: await pixelAudit(width, height) };
}

async function renderFrame(frame, width, height) {
  const sample = samplePath(frame);
  const camera = new THREE.PerspectiveCamera(sample.vertical_fov_deg, width / height, pathShot.camera.near_m, pathShot.camera.far_m);
  camera.position.fromArray(sample.position_m);
  camera.up.fromArray(pathShot.camera.up_vector);
  camera.lookAt(new THREE.Vector3().fromArray(sample.target_m));
  camera.updateProjectionMatrix();
  camera.updateMatrixWorld(true);
  renderWithCamera(camera, width, height);
  const audit = auditScene();
  ...
  const camera_collision = auditCameraCollision(camera);
  if (!camera_collision.pass) throw new Error(`Camera is inside: ${camera_collision.inside_placement_ids.join(", ")}`);
  return { shot_id: pathShot.shot_id, frame, width, height, sample, camera_collision, audit, pixels: await pixelAudit(width, height) };
}
