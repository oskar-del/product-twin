 ...
 *
 * Copied from repo-platform/scripts/shoot.mjs — Platform's script, their diagnosis. Kept here so
 * this repo can capture its own surfaces instead of asking another session to do it. If the
 * original changes, this is a copy and will not follow; the fleet rule against second copies is
 * about DATA, and a 90-line tool is cheaper to re-copy than to coordinate.
 *
 * The readiness flag for this repo's twin is `globalThis.twinReady`, set after loading.remove().
 * Start Chrome first:
...
const ready = arg("--ready", "globalThis.twinReady === true");
