/**
 * Build geometry/house-v0.3-geometry-spec.json — Vinkelhuset, room by room.
 *
 *   node geometry/build-house-v0.3.mjs
 *
...
const WING_ROOF = {
  ...
  falls_towards: "NORTH",
  wall_plate_Y: 3.0,
  eaves_overhang_m: 0.4,
  covering: "STANDING_SEAM_SHEET_METAL",
  note: "Cold wing. A shallower mono-pitch keeps the wing visually subordinate to the bar and sheds to the arrival court."
};
...
const spec = {
  spec_version: "brage-house-geometry/v0.3",
  entity_type: "RoomLevelHouseGeometrySpec",
  spec_id: "BRAGE_SE_SVARTINGE_54_28_HOUSE_V03",
  generated_by: "BRAGE (agent/brage-design) · geometry/build-house-v0.3.mjs",
  generated_at: new Date().toISOString(),
  supersedes: "brage-house-geometry/v0.2",
  subject: {
  ...
  evidence_policy: {
    ...
    derived_not_typed: "Every area, perimeter, volume, wall segment and the upper floor's usable width is computed by geometry/build-house-v0.3.mjs from footprint polygons and the roof section.",
    closed_gates: [
...
const out = path.join(here, "house-v0.3-geometry-spec.json");
fs.writeFileSync(out, `${JSON.stringify(spec, null, 2)}\n`);

console.log(`wrote geometry/house-v0.3-geometry-spec.json  ${(JSON.stringify(spec).length / 1024).toFixed(1)} KB`);
...
console.log(`  roof             ${ROOF.pitch_deg}° gable · ridge Y ${ROOF.ridge_Y} m · eaves Y ${ROOF.eaves_Y} m · wing ${WING_ROOF.pitch_deg}° mono`);
