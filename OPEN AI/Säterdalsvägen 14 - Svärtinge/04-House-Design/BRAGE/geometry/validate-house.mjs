/**
 * Validate a house geometry spec.
 *   node geometry/validate-house.mjs [house-v0.4-geometry-spec.json]
 *
 * Exits non-zero on any failure, so it can gate a commit.
 *
 * The roof-clearance check is the one that matters. v0.3 shipped a wing roof
 * that passed straight through the rooms below it and every other check here
 * still passed, because nothing related the roof to the rooms. Run this file
 * against house-v0.3-geometry-spec.json and it fails — that is the proof the
 * assertion works, not a claim that it does.
 */
...
const file = process.argv[2] ?? "house-v0.4-geometry-spec.json";
const spec = JSON.parse(fs.readFileSync(path.join(here, file), "utf8"));
const fail = [];
...
check(/^brage-house-geometry\/v0\.\d+$/.test(spec.spec_version ?? ""), "spec_version is not a brage-house-geometry version");
...
// Roof vs rooms — the check v0.3 did not have.
for (const f of roofClearanceFailures(spec)) {
  fail.push(`${f.room}: ceiling ${f.ceiling_m} m but the roof is only ${f.lowest_roof_m} m above datum at [${f.at_xz}] — short by ${f.shortfall_m} m`);
}

console.log(fail.length ? `FAIL ${file} (${fail.length})\n  ${fail.join("\n  ")}` : `PASS ${file} · ${spec.rooms.length} rooms · ${spec.rooms.reduce((s, r) => s + (r.openings?.length ?? 0), 0)} openings · heated ${heated.toFixed(1)} m² (brief ${recon.brief_figure_m2}, diff ${recon.difference_m2})`);
